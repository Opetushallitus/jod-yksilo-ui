...
const loader = (async ({ request, params }) => {
  ...
  if (paamaara) {
    ...
    let ammattiRyhmaData: components['schemas']['AmmattiryhmaBasicDto'] | undefined;
    ...
    if (paamaara.mahdollisuusTyyppi === 'TYOMAHDOLLISUUS') {
      ...
      ammattiRyhmaData = (await getTyoMahdollisuusDetails([m.id]).then((details) => details[0])).ammattiryhma;
      const ammattiryhmaUris = ammattiRyhmaData?.uri ? [ammattiRyhmaData.uri] : [];
    ...
    return { paamaara, mahdollisuus, vaaditutOsaamiset, profiiliOsaamiset, ammattiRyhmaData };
  }
