...
interface CvImportError {
  code: ErrorCode;
  detail?: string;
}

interface CvImportState {
  step: Step;
  tehtavaId?: string;
  tulos?: unknown;
  error?: CvImportError;
...
function reducer(state: CvImportState, action: CvImportAction): CvImportState {
  switch (action.type) {
    ...
    case 'POLL_DONE':
      return { step: 'done', tulos: action.tulos };
    case 'POLL_ERROR':
...
export const useCvUploadAndPoll = () => {
  ...
  const start = React.useCallback(async (file: File) => {
    ...
    try {
      ...
      if (tila === 'VALMIS') {
        dispatch({ type: 'POLL_DONE', tulos: tulos as unknown });
        return;
      }
  ...
  React.useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (data?.tila === 'VALMIS') {
          dispatch({ type: 'POLL_DONE', tulos: data.tulos as unknown });
