...
export const getEducationCodesetValues = async (ids: string[], getLatestVersion = true) => {
  ...
  const nonCachedIds = strippedIds.filter((id) => !cache.find((item) => item.code === id));
  ...
  nonCachedIds.forEach((id) => codesUrl.searchParams.append('koodiUris', id));
 ...
 * @returns TypedMahdollisuus array with yleisinKoulutusala codes replaces with labels
 */
